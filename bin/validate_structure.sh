...
# engine, the Lattice (the region co-locates its structure records WITH its runtime,
# which imports library(lattice)), the membership_contract (the region declares it),
# and the conformance harness. Exit 0 iff every record is valid and the cortisol skip
...
# Confirm the membership_contract construct the region rests on is reachable — a missing dependency is a failed build.
if [ ! -f "$PROLOGAI_HOME/packs/membership_contract/prolog/membership_contract.pl" ]; then
  echo "validate_structure.sh: cannot find PrologAI's library(membership_contract) at $PROLOGAI_HOME (set PROLOGAI_HOME)" >&2
  exit 2
fi
