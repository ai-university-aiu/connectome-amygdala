/*  connectome-amygdala — the Causalontology 2.0.0 structure validator.

    Assembles the region's newly-minted structure records from the five stratum
    packs (macromolecular, cellular, synaptic, region, community) and validates every
    one against PrologAI's UNMODIFIED Causalontology 2.0.0 conformance engine:
      ...
      - the cross-stratal CORTISOL skip CRO classifies as SKIPPING with NO skip-gap
        (community 14 -> macromolecular 4; skips:true, honest about the un-modeled
        hypothalamic-pituitary-adrenal pathway) — the Wave 2 slice's channel, re-engaged.
      - the signed provenance assertion over the cortisol skip VERIFIES (Ed25519).
    These are NEW records for the valence and salience region, grounded the way the
    earlier regions' were — not a copy. It writes each record to
    structure/<name>.json. Exit 0 iff every check passes.
*/

% Import the five stratum packs that each mint the region's structure records.
...
% -- validate_structure_records(-Records): assemble every record from the five stratum packs.
...
validate_structure_main :-
    % Print the banner.
    format("~n== connectome-amygdala :: Causalontology 2.0.0 structure validation ==~n~n", []),
    ...
    % Check the cross-stratal cortisol skip CRO classifies as skipping with no skip-gap.
    ...
    ->  length(Records, N),
        format("~nVALIDATION: PASS -- ~w records valid against Causalontology 2.0.0; cortisol skip finding and signature verified.~n~n", [N]),
        halt(0)
...
validate_one(record(Name, Kind, Dict), FailsIn, FailsOut) :-
    ...
    ->  format("  ok    ~w~t~46|~w~n", [Kind, Name]),
        FailsOut = FailsIn
    % The record failed one: print a FAIL line and record the reason.
    ;   format("  FAIL  ~w~t~46|~w  schema=~w why=~w sem=~w~n", [Kind, Name, SchemaOk, SchemaWhy, SemReasons]),
        FailsOut = [Name-schema(SchemaWhy)-semantics(SemReasons)|FailsIn] ).

% -- validate_skip(-Result): confirm the cortisol skip CRO is a skipping relation with no gap.
validate_skip(Result) :-
    % Classify the cross-stratal cortisol skip CRO and read its skip-gaps (from the community stratum pack).
    community_stratum_skip_check(Class, Gaps),
    % Print the finding.
    format("~n  cortisol skip finding: skip CRO classifies as ~w; skip-gaps=~w (community 14 -> macromolecular 4)~n", [Class, Gaps]),
...
validate_signature(Result) :-
    % Fetch the signed assertion over the cortisol skip CRO (from the community stratum pack).
    ...
    ->  format("  signature: Ed25519 assertion over the cortisol skip CRO VERIFIES~n", []),
        Result = ok
