...
# the region emit an undefined valence and confirms it cannot (the membership contract refuses a
# non-member valence). Exit 0 iff all pass.
set -u
...
# The region's recall declares the membership_contract construct, so it must be on the path.
